...
- Locked Hire gate until a paid task result and ERC-8183 receipt exist
...
The home, leaderboard, dashboard and registry detail pages use the same source-backed candidate model. The four selected services returned successful non-funded A2A negotiations, but no paid category task has been completed yet. Performance ranking and Hire remain unavailable until real results exist. The earlier self-transaction proof and representative performance dataset were removed from the user-facing product.
...
1. Run one paid category canary and inspect its result.
2. Replace the locked gate with an ERC-8183 job and funded $U escrow during Hire.
3. Grant and revoke an Altana session with a contract allowlist, token spend cap and expiry.
4. Track job status and deliverables in Dashboard.
5. Settle, dispute, refund or revoke from the user-facing controls.
